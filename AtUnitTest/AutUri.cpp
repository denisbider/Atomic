...
void FindAndCheckUris(Seq text, Seq prefixes, ParseTree::Storage* storage, Vec<Seq>& foundUris, Vec<Seq> const& expectUris, Display display)
{
	foundUris.ReserveExact(expectUris.Len());
	foundUris.Clear();
	Uri::FindUrisInText(text, prefixes, foundUris, storage);

	if (display == Display::Yes)
	{
		if (!foundUris.Any())
			Console::Out("No URIs found\r\n");
		else
		{
			Console::Out(Str("Found ").UInt(foundUris.Len()).Add(" URIs:\r\n"));
			for (sizet i=0; i!=foundUris.Len(); ++i)
				Console::Out(Str().UInt(i).Add(": ").Add(foundUris[i]).Add("\r\n"));
		}
	}
...
void UriTests()
{
	char const* const c_zText =
		...
		"HtTpS://example.com\r\n"
		"Here, a few links: https://example.com/, 'https://example.com/Path%20with%20escapes, \"http://example.com/with?semi;colon;\"\r\n"
		"Another one in parentheses (https://example.com/Path%20with%20escapes/and#fragment; but not this)\r\n"
		"not://alink.com\r\n"
		"\r\n"
	...
	Seq const c_prefixes = "http:// https:// mailto: ssh:// ftp://";

	enum { NrExpectUris = 15 };
	Vec<Seq> expectUris;
	...
	expectUris.Add("http://example.com/with?semi;colon;");
	...
	Vec<Seq> foundUris;
	FindAndCheckUris(c_text, c_prefixes, &storage, foundUris, expectUris, Display::Yes);
}
