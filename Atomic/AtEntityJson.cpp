...
namespace At
{
	...
	void Entity::JsonEncodeDynEntityOrNull(Enc& enc, Rp<Entity> const& o)
	{
		if (!o.Any())
			enc.Add("null");
		else
		{
			enc.Add("{\"k\":").Fun(JsonEncodeKind, o.Ref()).Add(",\"v\":");
			o->JsonEncode(enc);
			enc.Add("}");
		}
	}
	...
	void Entity::JsonEncodeFieldValue(Enc& a, EntityFieldInfo const& efi) const
	{
		...
		case FieldType::Time:     a.Ch('"').Obj(FieldRef<Time>(*this, efi.m_offset), TimeFmt::IsoMicroZ, 'T').Ch('"');	break;
		...
		case FieldType::OptDyn:   { Rp<Entity>    const& o(FieldRef<Rp<Entity   >>(*this, efi.m_offset)); if (o.Any()) JsonEncodeDynEntityOrNull(a, o);                         break; }
		case FieldType::OptEnt:   { EntOptBase    const& o(FieldRef<EntOptBase   >(*this, efi.m_offset)); if (o.Any()) o.Ref().JsonEncode(a);                                   break; } 
		case FieldType::OptBool:  { Opt<bool>     const& o(FieldRef<Opt<bool    >>(*this, efi.m_offset)); if (o.Any()) a.Add(If(o.Ref(), Seq, "true", "false"));                break; } 
		case FieldType::OptUInt:  { Opt<uint64>   const& o(FieldRef<Opt<uint64  >>(*this, efi.m_offset)); if (o.Any()) a.UInt(o.Ref());                                         break; } 
		case FieldType::OptSInt:  { Opt<int64>    const& o(FieldRef<Opt<int64   >>(*this, efi.m_offset)); if (o.Any()) a.SInt(o.Ref());                                         break; } 
		case FieldType::OptFloat: { Opt<double>   const& o(FieldRef<Opt<double  >>(*this, efi.m_offset)); if (o.Any()) a.Dbl(o.Ref());                                          break; } 
		case FieldType::OptTime:  { Opt<Time>     const& o(FieldRef<Opt<Time    >>(*this, efi.m_offset)); if (o.Any()) a.Ch('"').Obj(o.Ref(), TimeFmt::IsoMicroZ, 'T').Ch('"'); break; } 
		case FieldType::OptObjId: { Opt<ObjId>    const& o(FieldRef<Opt<ObjId   >>(*this, efi.m_offset)); if (o.Any()) a.Ch('"').Obj(o.Ref()).Ch('"');                          break; } 
		case FieldType::OptStr:   { Opt<Str>      const& o(FieldRef<Opt<Str     >>(*this, efi.m_offset)); if (o.Any()) JsonEncodeStrValue(a, o.Ref());                          break; } 
		...
		case FieldType::VecTime:  { Vec<Time>     const& v(FieldRef<Vec<Time    >>(*this, efi.m_offset)); Json::EncodeArray(a, v.Len(), [&] (Enc& a, sizet i) { a.Obj(v[i], TimeFmt::IsoMicroZ, 'T');  } ); break; }
	...
	void Entity::JsonDecodeFieldValue(ParseNode const& p, EntityFieldInfo const& efi, JsonIds* jsonIds)
	{
		...
		case FieldType::OptDyn:   { Rp<Entity>&    o(FieldRef<Rp<Entity   >>(*this, efi.m_offset)); JsonDecodeDynEntityOrNull(p, o);        break; }
		case FieldType::OptEnt:   { EntOptBase&    o(FieldRef<EntOptBase   >(*this, efi.m_offset)); o.Init().JsonDecode(p, jsonIds);        break; }
		case FieldType::OptBool:  { Opt<bool  >&   o(FieldRef<Opt<bool    >>(*this, efi.m_offset)); o.Init() = Json::DecodeBool  (p);       break; }
		case FieldType::OptUInt:  { Opt<uint64>&   o(FieldRef<Opt<uint64  >>(*this, efi.m_offset)); o.Init() = Json::DecodeUInt  (p);       break; }
		case FieldType::OptSInt:  { Opt<int64 >&   o(FieldRef<Opt<int64   >>(*this, efi.m_offset)); o.Init() = Json::DecodeSInt  (p);       break; }
		case FieldType::OptFloat: { Opt<double>&   o(FieldRef<Opt<double  >>(*this, efi.m_offset)); o.Init() = Json::DecodeFloat (p);       break; }
		case FieldType::OptTime:  { Opt<Time  >&   o(FieldRef<Opt<Time    >>(*this, efi.m_offset)); o.Init() = Json::DecodeTime  (p);       break; }
		case FieldType::OptObjId: { Opt<ObjId >&   o(FieldRef<Opt<ObjId   >>(*this, efi.m_offset)); o.Init() = JsonDecodeObjId(p, jsonIds); break; }
		case FieldType::OptStr:   { Opt<Str   >&   o(FieldRef<Opt<Str     >>(*this, efi.m_offset)); JsonDecodeStrValue(p, o.Init());        break; }

		case FieldType::VecDyn:   { RpVec<Entity>& v(FieldRef<RpVec<Entity>>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { JsonDecodeDynEntityOrNull(p, v.Add()); } ); break; }
		case FieldType::VecEnt:   { EntVecBase&    v(FieldRef<EntVecBase   >(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add().JsonDecode(p, jsonIds);        } ); break; }
		case FieldType::VecBool:  { Vec<bool  >&   v(FieldRef<Vec<bool    >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add() = Json::DecodeBool  (p);       } ); break; }
		case FieldType::VecUInt:  { Vec<uint64>&   v(FieldRef<Vec<uint64  >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add() = Json::DecodeUInt  (p);       } ); break; }
		case FieldType::VecSInt:  { Vec<int64 >&   v(FieldRef<Vec<int64   >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add() = Json::DecodeSInt  (p);       } ); break; }
		case FieldType::VecFloat: { Vec<double>&   v(FieldRef<Vec<double  >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add() = Json::DecodeFloat (p);       } ); break; }
		case FieldType::VecTime:  { Vec<Time  >&   v(FieldRef<Vec<Time    >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add() = Json::DecodeTime  (p);       } ); break; }
		case FieldType::VecObjId: { Vec<ObjId >&   v(FieldRef<Vec<ObjId   >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { v.Add() = JsonDecodeObjId(p, jsonIds); } ); break; }
		case FieldType::VecStr:   { Vec<Str   >&   v(FieldRef<Vec<Str     >>(*this, efi.m_offset)); Json::DecodeArray(p, [&] (ParseNode const& p) { JsonDecodeStrValue(p, v.Add());        } ); break; }
