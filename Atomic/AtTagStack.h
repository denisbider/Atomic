...
namespace At
{
	...
	public:
		...
		void PopExact (Seq tag) { EnsureThrow(m_tags.Any()); Seq last { Last() }; if (last != tag) PopTagMismatch(last, tag); m_tags.ResizeExact(SatSub(m_tags.Len(), last.n + 1)); }
		void Pop      ()        { EnsureThrow(m_tags.Any()); Seq last { Last() }; m_tags.ResizeExact(SatSub(m_tags.Len(), last.n + 1)); }
	...
	private:
		...
		void PopTagMismatch(Seq expected, Seq actual);
	};
